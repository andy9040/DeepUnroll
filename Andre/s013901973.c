...
long long prim(long long vc,long long ec,rs g[],mkj x[]){
  resheap();
  data pd,od;
  long long cnt=0,i,r=0,nv;
  bool fl[1048576]={0};
  pd.node=1;
  pd.cost=0;
  pqpush(pd);
...
int main(void){
    ...
    long long ec,vc;
    sd xa[524288],ya[524288];
    rs g[1048576];
    mkj x[1048576];
    scanf("%lld",&n);
    vc=n;
    for(i=1;i<=n;i++){
      scanf("%lld%lld",&xa[i].val,&ya[i].val);
      xa[i].node=i;
      ya[i].node=i;
    }
    ec=0;
    qsort(&xa[1],n,sizeof(xa[1]),sdsortfnc);
    qsort(&ya[1],n,sizeof(ya[1]),sdsortfnc);
    for(i=2;i<=n;i++){
      g[ec].st=xa[i-1].node;
      g[ec].fi=xa[i].node;
      g[ec].kr=xa[i].val-xa[i-1].val;
      ec++;
      g[ec].fi=xa[i-1].node;
      g[ec].st=xa[i].node;
      g[ec].kr=xa[i].val-xa[i-1].val;
