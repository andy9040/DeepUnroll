...
static uint64_t pf[10000];
static uint64_t pfListSize = 0;
static uint64_t dp[10000001];

static uint64_t getR(uint64_t v) {
    uint64_t r = 1;
    uint64_t d = 2;
    const uint64_t lim = (uint64_t)sqrt((double)(v + 1));
...
int main(void) {
    ...
    for (uint64_t a = 0, av = 1; av <= N; a++, av *= 2) {
        for (uint64_t b = 0, bv = 1; av * bv <= N; b++, bv *= 3) {
            for (uint64_t c = 0, cv = 1; av * bv * cv <= N; c++, cv *= 5) {
                for (uint64_t d = 0, dv = 1; av * bv * cv * dv <= N; d++, dv *= 7) {
                    for (uint64_t e = 0, ev = 1; av * bv * cv * dv * ev <= N; e++, ev *= 11) {
                        dp[av * bv * cv * dv * ev] = (a + 1) * (b + 1) * (c + 1) * (d + 1) * (e + 1);
                    }
                }
            }
        }
